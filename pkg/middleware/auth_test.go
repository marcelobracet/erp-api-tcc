...
func TestAuthMiddleware_Authenticate_ValidToken(t *testing.T) {
	...
	// Gerar token válido
	token, err := jwtManager.GenerateAccessToken("user-123", "tenant-123", "test@example.com", "user")
	if err != nil {
...
func TestAuthMiddleware_RequireRole_Success(t *testing.T) {
	...
	token, err := jwtManager.GenerateAccessToken("user-123", "tenant-123", "test@example.com", "admin")
	if err != nil {
		t.Fatalf("Failed to generate token: %v", err)
	}
...
func TestAuthMiddleware_RequireAnyRole_Success(t *testing.T) {
	...
	token, err := jwtManager.GenerateAccessToken("user-123", "tenant-123", "test@example.com", "manager")
	if err != nil {
		t.Fatalf("Failed to generate token: %v", err)
	}
